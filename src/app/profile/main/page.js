  ...
  const onClickSave = async () => {
    ...
    if (response.data.success) {
      ...
      localStorage.setItem(
        "user-SattyTatty",
        JSON.stringify({ ...user, fullname: response.data.user.fullname }),
      );
    } else {
          ...
          if (newData?.accountType === "provider") {
            const response = await axios.get(
              process.env.NEXT_PUBLIC_SERVER_URL +
                "/users/getProviderInfoByUserId/" +
                newData.id,
            );
            if (response.data.success) {
              const a = response.data.info.workDays
                ? response.data.info.workDays.split(",")
                : null;
              setSelectedWorkDays(
                response.data.info.workDays
                  ? response.data.info.workDays.split(",")
                  : null,
              );
          ...
          {isProvider ? (
            <label>
              ...
              <Select
                ...
                defaultValue={
                  providerInfo.workDays
                    ? providerInfo.workDays.split(",").map((item) => ({
                        value: item,
                        label: WORK_DAYS[item],
                      }))
                    : null
                }
              />
